...
public class UpdateUtils {
    ...
    static String getLocalApkDownSavePath(String apkName){
     ...
     /*   String sdPath = getInnerSDCardPath();
        ...
        saveApkPath = saveApkPath + apkName+".apk";
        Log.e("getLocalApkDownSavePath", "getLocalApkDownSavePath: "+saveApkPath );
        return saveApkPath;
    }
...
//        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState()) ) {//判断外部存储是否可用
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            directoryPath = Objects.requireNonNull(context.getExternalFilesDir("")).getAbsolutePath();
        }
        /*}else{//没外部存储就使用内部存储
