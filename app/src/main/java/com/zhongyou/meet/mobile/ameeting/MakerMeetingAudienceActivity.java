...
public class MakerMeetingAudienceActivity extends BaseActivity implements AGEventHandler {

	private final String TAG = MakerMeetingAudienceActivity.class.getSimpleName();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_meeting_audience_maker);
...
//						mLogger.e("left="+l+"   top="+t+"    right="+r+"   bottom="+b);
	...
	protected void initUIandEvent() {
		...
		if (meetingJoin == null || meetingJoin.getHostUser() == null) {
			...
			dialogBuilder
					.withTitle("发生了意外")
					.withMessage("没有获取到主持人信息 请重新进入会议")
		...
		audienceName = MMKV.defaultMMKV().decodeString(MMKVHelper.ADDRESS, "")
				+ "-" + MMKV.defaultMMKV().decodeString(MMKVHelper.POSTTYPENAME, "") + "-" + Preferences.getUserName();
		...
		requestTalkButton.setOnClickListener(view -> {
			if (remoteBroadcasterSurfaceView != null) {
				...
				Toast.makeText(MakerMeetingAudienceActivity.this, "主持人加入才能申请发言", Toast.LENGTH_SHORT).show();
			}
		});
		...
		if (Constant.videoType == 2) {
			mLogger.e(config().mUid);
			worker().configEngine(Constants.CLIENT_ROLE_AUDIENCE, VideoEncoderConfiguration.VD_320x240);//参会人是320*180
			...
			worker().configEngine(Constants.CLIENT_ROLE_BROADCASTER, VideoEncoderConfiguration.VD_320x240);//参会人是320*180
//			rtcEngine().enableAudioVolumeIndication(400, 3);
		...
		agoraAPI.setAttr("userName", audienceName);
		...
		agoraAPI.callbackSet(new AgoraAPI.CallBack() {
			...
			public void onLoginSuccess(int uid, int fd) {
				...
				if (BuildConfig.DEBUG) {
					runOnUiThread(() -> Toast.makeText(MakerMeetingAudienceActivity.this, "观众登录信令系统成功", Toast.LENGTH_SHORT).show());
				}
			...
			public void onLoginFailed(final int ecode) {
				...
				if (BuildConfig.DEBUG) {
					runOnUiThread(() -> Toast.makeText(MakerMeetingAudienceActivity.this, "观众登录信令系统失败" + ecode, Toast.LENGTH_SHORT).show());
				}
			...
			public void onLogout(int ecode) {
						...
						runOnUiThread(new Runnable() {
							...
							public void run() {
								if (mSignalDialog == null) {
									if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
										if (MakerMeetingAudienceActivity.this.isDestroyed()) {
											return;
										}
									}
									if (MakerMeetingAudienceActivity.this.isFinishing()) {
										return;
									}
									mSignalDialog = new AlertDialog(MakerMeetingAudienceActivity.this)
											.builder()
											.setTitle("登陆系统失败")
				...
				if (BuildConfig.DEBUG) {
					runOnUiThread(() -> Toast.makeText(MakerMeetingAudienceActivity.this, "观众登出信令系统成功" + ecode, Toast.LENGTH_SHORT).show());
				}
			}
			...
			public void onChannelJoined(String channelID) {
				...
				runOnUiThread(() -> {
					if (BuildConfig.DEBUG) {
						Toast.makeText(MakerMeetingAudienceActivity.this, "观众登录信令频道成功", Toast.LENGTH_SHORT).show();
					}
			...
			public void onReconnecting(int nretry) {
				...
				if (BuildConfig.DEBUG) {
					runOnUiThread(() -> Toast.makeText(MakerMeetingAudienceActivity.this, "信令重连失败第" + nretry + "次", Toast.LENGTH_SHORT).show());
				}
			}
			...
			public void onReconnected(int fd) {
				...
				if (BuildConfig.DEBUG) {
					runOnUiThread(() -> Toast.makeText(MakerMeetingAudienceActivity.this, "信令系统重连成功", Toast.LENGTH_SHORT).show());
				}
				runOnUiThread(new Runnable() {
					@Override
					public void run() {
			...
			public void onChannelJoinFailed(String channelID, int ecode) {
				...
				runOnUiThread(() -> {
					if (BuildConfig.DEBUG) {
						Toast.makeText(MakerMeetingAudienceActivity.this, "观众登录信令频道失败", Toast.LENGTH_SHORT).show();
					}
				});
			}
			...
			public void onUserAttrResult(final String account, final String name, final String value) {
				...
				runOnUiThread(() -> {
					if (BuildConfig.DEBUG) {
						Toast.makeText(MakerMeetingAudienceActivity.this, "获取到用户" + account + "的属性" + name + "的值为" + value, Toast.LENGTH_SHORT).show();
					}
			...
			public void onChannelUserLeaved(String account, int uid) {
				...
				runOnUiThread(() -> {
					try {
						if (BuildConfig.DEBUG) {
							Toast.makeText(MakerMeetingAudienceActivity.this, "用户" + account + "退出信令频道", Toast.LENGTH_SHORT).show();
						}
			...
			public void onMessageSendSuccess(String messageID) {
				...
				if (BuildConfig.DEBUG) {
					runOnUiThread(() -> {
						Toast.makeText(MakerMeetingAudienceActivity.this, "发送成功", Toast.LENGTH_SHORT).show();
					});
				}
			}
			...
			public void onMessageSendError(String messageID, int ecode) {
				...
				if (BuildConfig.DEBUG) {
					runOnUiThread(() -> {
						Toast.makeText(MakerMeetingAudienceActivity.this, "发送失败", Toast.LENGTH_SHORT).show();
					});
				}
			}
			...
			public void onMessageInstantReceive(final String account, final int uid, final String msg) {
				...
				runOnUiThread(() -> {
					try {
						if (BuildConfig.DEBUG) {
							Toast.makeText(MakerMeetingAudienceActivity.this, "接收到消息" + msg, Toast.LENGTH_SHORT).show();
						}
						...
						if (jsonObject.has("response")) {
							...
							if (result) { // 连麦成功
								if (BuildConfig.DEBUG) {
									Toast.makeText(MakerMeetingAudienceActivity.this, "主持人要和我连麦", Toast.LENGTH_SHORT).show();
								}
								...
								if (BuildConfig.DEBUG) {
									Toast.makeText(MakerMeetingAudienceActivity.this, "主持人拒绝和我连麦", Toast.LENGTH_SHORT).show();
								}
...
//								}
			...
			public void onMessageChannelReceive(String channelID, String account, int uid, final String msg) {
				...
				runOnUiThread(() -> {
					try {
						if (BuildConfig.DEBUG) {
							Toast.makeText(MakerMeetingAudienceActivity.this, "接收到频道消息：" + msg, Toast.LENGTH_SHORT).show();
						}
						JSONObject jsonObject = new JSONObject(msg);
						Logger.e(jsonObject.toString());
							...
							if (currentMaterial != null) {
								if (!mMaterialId.equals(currentMaterial.getId())) {
									...
									if (currentMaterialPublish.getType().equals("1")) {
										...
										Glide.with(MakerMeetingAudienceActivity.this).load(currentMaterialPublish.getUrl())
						...
						if (jsonObject.has("finish_meeting")) {
							...
							if (finishMeeting) {
								if (BuildConfig.DEBUG) {
									Toast.makeText(MakerMeetingAudienceActivity.this, "主持人结束了会议", Toast.LENGTH_SHORT).show();
								}
								doLeaveChannel();
...
//								agoraAPI.setAttr("uname", null);
							...
							if (BuildConfig.DEBUG) {
								Toast.makeText(MakerMeetingAudienceActivity.this, "收到主持人设置的连麦人ID：" + value + ", \ntype:" + type, Toast.LENGTH_SHORT).show();
							}
						...
						if (!TextUtils.isEmpty(value)) {
							...
							try {
								...
								if (jsonObject.has("material_id") && jsonObject.has("doc_index")) {
									...
									doc_index = jsonObject.getInt("doc_index");
									if (BuildConfig.DEBUG) {
										Toast.makeText(MakerMeetingAudienceActivity.this, "收到主持人端index：" + doc_index, Toast.LENGTH_SHORT).show();
									}
									mMaterialId = jsonObject.getString("material_id");
								/*	mAudienceRecyclerView.setVisibility(View.GONE);
									mVideoAdapter.setVisibility(View.GONE);*/
...
//								player.startButton.performClick();
			...
			public void onError(final String name, final int ecode, final String desc) {
				...
				if (BuildConfig.DEBUG) {
					runOnUiThread(() -> {
						if (ecode != 208)
							Toast.makeText(MakerMeetingAudienceActivity.this, "收到错误信息\nname: " + name + "\necode: " + ecode + "\ndesc: " + desc, Toast.LENGTH_SHORT).show();
					});
				}
				if (ecode == 1002) {
					return;
			...
			public void onSuccess(Bucket<MeetingJoin.DataBean.HostUserBean> meetingJoinBucket) {
				...
				if (uid != 0 && broadcastId != null) {
					...
					if (String.valueOf(uid).equals(broadcastId)) {
						if (BuildConfig.DEBUG) {
							mLogger.e("主持人进入");
							Toast.makeText(MakerMeetingAudienceActivity.this, "主持人" + broadcastId + "---" + uid + meetingJoin.getHostUser().getHostUserName() + "进入了", Toast.LENGTH_SHORT).show();
						}
						model = -1;
						isHostCommeIn = true;
						agoraAPI.channelJoin(channelName);
						...
						if (BuildConfig.DEBUG) {
							Toast.makeText(MakerMeetingAudienceActivity.this, "参会人" + uid + "正在连麦", Toast.LENGTH_SHORT).show();
						}
...
//						audienceView.addView(remoteAudienceSurfaceView);
			...
			public void onFailure(int errorCode, BaseException exception) {
				super.onFailure(errorCode, exception);
				Toast.makeText(MakerMeetingAudienceActivity.this, exception.getMessage(), Toast.LENGTH_SHORT).show();
			}
		};
	}
...
//				mPlayVideoText.setVisibility(View.GONE);
				...
				pageText.setText("第" + currentMaterialPublish.getPriority() + "/" + currentMaterial.getMeetingMaterialsPublishList().size() + "页");
				docImage.setVisibility(View.VISIBLE);
				Glide.with(MakerMeetingAudienceActivity.this)
		...
		public void onFailure(int errorCode, BaseException exception) {
			super.onFailure(errorCode, exception);
			Toast.makeText(MakerMeetingAudienceActivity.this, errorCode + "---" + exception.getMessage(), Toast.LENGTH_SHORT).show();
		}
	};
	...
	public void onUserOffline(int uid, int reason) {
		...
		runOnUiThread(() -> {
			...
			if (String.valueOf(uid).equals(broadcastId)) {
				...
				if (BuildConfig.DEBUG) {
					Toast.makeText(MakerMeetingAudienceActivity.this, "连麦观众" + uid + "退出了" + config().mUid, Toast.LENGTH_SHORT).show();
				}
...
//					mVideoAdapter.getAudienceVideoLists().remove(mVideoAdapter.getPositionById(uid));
	...
	public void onConnectionInterrupted() {
		runOnUiThread(() -> Toast.makeText(MakerMeetingAudienceActivity.this, "网络连接不佳，视频将会有卡顿，可尝试降低分辨率", Toast.LENGTH_SHORT).show());
	}

	@Override
	public void onUserMuteVideo(final int uid, final boolean muted) {
		if (BuildConfig.DEBUG) {
			runOnUiThread(() -> Toast.makeText(MakerMeetingAudienceActivity.this, uid + " 的视频被暂停了 " + muted, Toast.LENGTH_SHORT).show());
		}
	}
...
//        runOnUiThread(() -> Toast.makeText(MeetingAudienceActivity.this, "警告：" + warn, Toast.LENGTH_SHORT).show());
	...
	public void onError(final int err) {
		runOnUiThread(() -> Toast.makeText(MakerMeetingAudienceActivity.this, "发生错误的错误码：" + err, Toast.LENGTH_SHORT).show());
	...
	public void onRemoteVideoStateChanged(int uid, int state,int i ,int x) {
		...
		runOnUiThread(new Runnable() {
			...
			/*	if (state == 2) {
					...
					if (String.valueOf(uid).equals(broadcastId)) {
						Toasty.warning(MeetingAudienceActivity.this, "主持人的视频卡顿", Toasty.LENGTH_LONG, true).show();
					}
				} else */
				if (state == 4) {//远端视频流播放失败
					...
					if (String.valueOf(uid).equals(broadcastId)) {
						Toasty.warning(MakerMeetingAudienceActivity.this, "主持人视频的加载失败", Toasty.LENGTH_LONG, true).show();
					}
				}
			}
		});
...
//							agoraAPI.setAttr("uname", null);
	...
	public void PlayVideo() {
		...
		String imageUrl = ImageHelper.videoFrameUrl(currentMaterial.getMeetingMaterialsPublishList().get(0).getUrl()
				, AutoSizeUtils.dp2px(MakerMeetingAudienceActivity.this, 300)
				, AutoSizeUtils.dp2px(MakerMeetingAudienceActivity.this, 400));
		Glide.with(MakerMeetingAudienceActivity.this).load(imageUrl)
