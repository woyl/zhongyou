...
class ClassRoomCloundFragment : BaseLazyLoadFragment<IPresenter>() {
    ...
    companion object {
        fun newInstance() :ClassRoomCloundFragment?{
            return ClassRoomCloundFragment()
        }
    }
    ...
    override fun initData(savedInstanceState: Bundle?) {
        ...
        smart_refresh?.setOnRefreshListener {
            curPosition = 1
            presenter?.getMeetings(mCurrentMeetingType, curPosition, "","1")
        }
    ...
    fun setData (meetingsData: MeetingsData?) {
...
//                            Glide.with(mContext).load(it.lecturerHeadUrl).apply(options).placeholder(R.drawable.defaule_banner).error(R.drawable.defaule_banner).into(img_small_head)
//                            tvName.text = it.lecturerInfo
                            ...
                            if (it.meetingProcess == 1) {
                                Glide.with(mContext).load(R.mipmap.cloud_starting).into(imgState)
                            } else {
                            ...
                            view.itemView.setOnClickListener { view ->
                                ...
                                try {
                                    if (list[position].isToken.toDouble() == 1.0) { //需要加入码的
                                        if (activity != null) {
                                            JoinMeetingDialog.showNeedCodeDialog((activity as AppCompatActivity?)!!, list[position].id)
                                        }
                                    } else {
                                        if (activity != null) {
                                            JoinMeetingDialog.showNoCodeDialog((activity as AppCompatActivity?)!!, list[position].id)
                                        }
                                    }
                                } catch (e: Exception) {
                                    if (activity != null) {
    ...
    fun getCloudAdapter() : BaseRecyclerViewAdapter<MeetingsData.DataBean.ListBean>? {
        return mAdapter
    }
