		...
		try {
			if (response.body() != null) {
				...
				if (jsonObject.containsKey("errcode")) {
					if (jsonObject.getInteger("errcode") == 40001 || jsonObject.getInteger("errcode") == 40003) {
						Preferences.clear();
						MMKVHelper.getInstance().loginOut();
						context.startActivity(new Intent(context, WXEntryActivity.class).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK).putExtra("isBadToken", true));
					}
				}
